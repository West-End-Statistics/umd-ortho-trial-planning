...
placebo_status <- get_amb_status2(
  latent_variable
)
active_status <- get_amb_status2(
  active_variable
)
...
simulate_data <- function(n, arm = c("placebo", "active"), outcome_correlation = 0) {
  ...
  out <- data.frame(
    ...
    ambulation_status = get_amb_status2(underlying_data[, 3])
  )
...
simulate_data2 <- function(n, arm = c("placebo", "active"), outcome_correlation = 0) {
  arm <- match.arg(arm)
...
simulate_trial <- function(n_per_arm, outcome_correlation = 0, method = 1) {
  if (method == 1) {
    out <- dplyr::bind_rows(
    ...
    out <- dplyr::bind_rows(
      placebo = simulate_data2(n_per_arm, arm = "placebo", outcome_correlation = outcome_correlation),
      active = simulate_data2(n_per_arm, arm = "active", outcome_correlation = outcome_correlation),
      .id = "arm"
    )
  }
...
set.seed(20240515)
cur_method <- 2
trial_no_corr <- simulate_trial(10000, outcome_correlation = 0, method = cur_method)
trial_hi_corr <- simulate_trial(10000, outcome_correlation = .9, method = cur_method)
trial_cust_corr <- simulate_trial(10000, outcome_correlation = custom_outcome_corr(.3, .5, -.5), method = cur_method)

d_no_corr <- simulate_data2(10000, outcome_correlation = 0)
d_hi_corr <- simulate_data2(10000, outcome_correlation = .9)
d_cust_corr <- simulate_data2(10000, outcome_correlation = custom_outcome_corr(.3, .5, -.5))
...
```{r}
set.seed(2024051)
no_corr_simple_trial <- simulate_trial(n = 150, outcome_correlation = 0)
